...
public final class ITFeesExchange {
    ...
    public void shouldRequireTakerFees_GtcCancel() throws Exception {
        ...
        try (final ExchangeTestContainer container = new ExchangeTestContainer()) {
            ...
            container.createUserWithMoney(UID_2, CURRENCY_LTC, ltcAmount);
            ...
            // add fee-1 - NSF
            container.addMoneyToUser(UID_2, CURRENCY_LTC, takerFee * 30 - 1);
            container.submitCommandSync(order203, CommandResultCode.RISK_NSF);

            // add 1 extra - SUCCESS
            container.addMoneyToUser(UID_2, CURRENCY_LTC, 1);
            ...
            container.validateUserState(
                    UID_2,
                    userProfile -> assertThat(userProfile.accounts.get(CURRENCY_LTC), is(ltcAmount + takerFee * 30)),
                    orders -> assertTrue(orders.isEmpty()));

            TotalCurrencyBalanceReportResult totalBal1 = container.totalBalanceReport();
            assertThat(totalBal1.getSum().get(CURRENCY_LTC), is(ltcAmount + takerFee * 30));
            assertThat(totalBal1.getFees().get(CURRENCY_LTC), is(0L));

            // ----------------- 2 test GTC ASK cancel ------------------

            // add 100M satoshi (1 BTC)
            final long btcAmount = 100_000_000L;
            container.addMoneyToUser(UID_2, CURRENCY_XBT, btcAmount);
            ...
            container.validateUserState(
                    UID_2,
                    userProfile -> assertThat(userProfile.accounts.get(CURRENCY_XBT), is(btcAmount)),
                    orders -> assertTrue(orders.isEmpty()));

            // no fees collected
            TotalCurrencyBalanceReportResult totalBal2 = container.totalBalanceReport();
            assertThat(totalBal2.getSum().get(CURRENCY_LTC), is(ltcAmount + takerFee * 30));
            assertThat(totalBal2.getSum().get(CURRENCY_XBT), is(btcAmount));
            assertThat(totalBal2.getFees().get(CURRENCY_LTC), is(0L));
            assertThat(totalBal2.getFees().get(CURRENCY_XBT), is(0L));
        }
    }
    ...
    public void shouldProcessFees_BidGtcMaker_AskIocTakerPartial() throws Exception {
        ...
        try (final ExchangeTestContainer container = new ExchangeTestContainer()) {
            ...
            container.createUserWithMoney(UID_1, CURRENCY_LTC, ltcAmount); // 200B litoshi (2,000 LTC)
            ...
            container.validateUserState(
                    UID_1,
                    userProfile -> assertThat(userProfile.accounts.get(CURRENCY_LTC), is(expectedFundsLtc)),
                    orders -> assertThat(orders.get(101L).price, is(order101.price)));

            // create second user
            final long btcAmount = 2_000_000_000L;
            container.createUserWithMoney(UID_2, CURRENCY_XBT, btcAmount);

            // no fees collected
            TotalCurrencyBalanceReportResult totalBal1 = container.totalBalanceReport();
            assertThat(totalBal1.getSum().get(CURRENCY_LTC), is(ltcAmount));
            assertThat(totalBal1.getSum().get(CURRENCY_XBT), is(btcAmount));
            assertThat(totalBal1.getFees().get(CURRENCY_LTC), is(0L));
            ...
            container.validateUserState(
                    UID_1,
                    userProfile -> {
                        assertThat(userProfile.accounts.get(CURRENCY_LTC), is(ltcAmount - (order101.price * step + makerFee) * 1731L));
                        assertThat(userProfile.accounts.get(CURRENCY_XBT), is(1731L * SYMBOLSPECFEE_XBT_LTC.baseScaleK));
                    },
                    orders -> assertTrue(orders.isEmpty()));
            ...
            container.validateUserState(
                    UID_2,
                    userProfile -> {
                        assertThat(userProfile.accounts.get(CURRENCY_LTC), is((order101.price * step - takerFee) * 1731L));
                        assertThat(userProfile.accounts.get(CURRENCY_XBT), is(btcAmount - 1731L * SYMBOLSPECFEE_XBT_LTC.baseScaleK));
                    },
                    orders -> assertTrue(orders.isEmpty()));

            // total balance remains the same
            final TotalCurrencyBalanceReportResult totalBal2 = container.totalBalanceReport();
            assertThat(totalBal2.getSum().get(CURRENCY_LTC), is(ltcAmount));
            assertThat(totalBal2.getSum().get(CURRENCY_XBT), is(btcAmount));
            assertThat(totalBal2.getFees().get(CURRENCY_LTC), is((makerFee + takerFee) * 1731L));
        }
    ...
    public void shouldProcessFees_BidGtcMakerPartial_AskIocTaker() throws Exception {
        ...
        try (final ExchangeTestContainer container = new ExchangeTestContainer()) {
            ...
            container.validateUserState(
                    UID_1,
                    userProfile -> {
                        assertThat(userProfile.accounts.get(CURRENCY_LTC),
                                is(ltcAmount - (order101.price * step + makerFee) * 1000L - (order101.reservePrice * step + takerFee) * 731L));
                        assertThat(userProfile.accounts.get(CURRENCY_XBT), is(1000L * SYMBOLSPECFEE_XBT_LTC.baseScaleK));
                    },
            ...
            container.validateUserState(
                    UID_2,
                    userProfile -> {
                        assertThat(userProfile.accounts.get(CURRENCY_LTC), is((order101.price * step - takerFee) * 1000L));
                        assertThat(userProfile.accounts.get(CURRENCY_XBT), is(btcAmount - 1000L * SYMBOLSPECFEE_XBT_LTC.baseScaleK));
                    },
                    orders -> assertTrue(orders.isEmpty()));
            ...
            assertThat(totalBal2.getSum().get(CURRENCY_LTC), is(ltcAmount));
            assertThat(totalBal2.getSum().get(CURRENCY_XBT), is(btcAmount));
            assertThat(totalBal2.getFees().get(CURRENCY_LTC), is((makerFee + takerFee) * 1000L));
        }
    ...
    public void shouldProcessFees_AskGtcMaker_BidIocTakerPartial() throws Exception {
        ...
        try (final ExchangeTestContainer container = new ExchangeTestContainer()) {
            ...
            final long btcAmount = 2_000_000_000L;
            container.createUserWithMoney(UID_1, CURRENCY_XBT, btcAmount);
            ...
            container.validateUserState(
                    ...
                    userProfile -> {
                        ...
                        assertThat(userProfile.accounts.get(CURRENCY_LTC), is((11_500L * step - makerFee) * 2000L));
                    },
                    orders -> assertTrue(orders.isEmpty()));
            ...
            container.validateUserState(
                    UID_2,
                    userProfile -> {
                        assertThat(userProfile.accounts.get(CURRENCY_XBT), is(SYMBOLSPECFEE_XBT_LTC.baseScaleK * 2000L));
                        assertThat(userProfile.accounts.get(CURRENCY_LTC), is(ltcAmount - (11_500L * step + takerFee) * 2000L));
                    },
                    orders -> assertTrue(orders.isEmpty()));
            ...
            assertThat(totalBal2.getSum().get(CURRENCY_LTC), is(ltcAmount));
            assertThat(totalBal2.getSum().get(CURRENCY_XBT), is(btcAmount));
            assertThat(totalBal2.getFees().get(CURRENCY_LTC), is((makerFee + takerFee) * 2000L));
        }
    ...
    public void shouldProcessFees_AskGtcMakerPartial_BidIocTaker() throws Exception {
        ...
        try (final ExchangeTestContainer container = new ExchangeTestContainer()) {
            ...
            container.validateUserState(
                    ...
                    userProfile -> {
                        assertThat(userProfile.accounts.get(CURRENCY_XBT), is(0L));
                        assertThat(userProfile.accounts.get(CURRENCY_LTC), is((11_500L * step - makerFee) * 1997L));
                    },
                    orders -> assertFalse(orders.isEmpty()));
            ...
            container.validateUserState(
                    UID_2,
                    userProfile -> {
                        assertThat(userProfile.accounts.get(CURRENCY_XBT), is(SYMBOLSPECFEE_XBT_LTC.baseScaleK * 1997L));
                        assertThat(userProfile.accounts.get(CURRENCY_LTC), is(ltcAmount - (11_500L * step + takerFee) * 1997L));
                    },
                    orders -> assertTrue(orders.isEmpty()));
            ...
            assertThat(totalBal2.getSum().get(CURRENCY_LTC), is(ltcAmount));
            assertThat(totalBal2.getSum().get(CURRENCY_XBT), is(btcAmount));
            assertThat(totalBal2.getFees().get(CURRENCY_LTC), is((makerFee + takerFee) * 1997L));
        }
    }
