...
import kmi.exchange.core.ExchangeCore;
...
import kmi.exchange.core.ExchangeApi;
import kmi.exchange.core.ExchangeCore;
import kmi.exchange.core.journalling.DiskSerializationProcessor;
...
import static kmi.exchange.core.ExchangeCore.DisruptorWaitStrategy.BUSY_SPIN;
import static kmi.exchange.core.Utils.ThreadAffityMode.THREAD_AFFINITY_ENABLE_PER_LOGICAL_CORE;
...
public final class ExchangeTestContainer implements AutoCloseable {
    ...
    public ExchangeTestContainer(final int bufferSize,
        ...
        this.exchangeCore = ExchangeCore.builder()
                ...
                .threadAffityMode(Utils.ThreadAffityMode.THREAD_AFFINITY_ENABLE_PER_LOGICAL_CORE)
                .waitStrategy(ExchangeCore.DisruptorWaitStrategy.BUSY_SPIN)
...
//    }
    ...
    public void initBasicUsers() throws InterruptedException {
        ...
        cmds.add(ApiAdjustUserBalance.builder().uid(TestConstants.UID_1).transactionId(1L).amount(10_000_00L).currency(TestConstants.CURRENCY_USD).build());
        cmds.add(ApiAdjustUserBalance.builder().uid(TestConstants.UID_1).transactionId(2L).amount(1_0000_0000L).currency(TestConstants.CURRENCY_XBT).build());
        cmds.add(ApiAdjustUserBalance.builder().uid(TestConstants.UID_1).transactionId(3L).amount(1_0000_0000L).currency(TestConstants.CURRENCY_ETH).build());

        cmds.add(ApiAddUser.builder().uid(TestConstants.UID_2).build());
        cmds.add(ApiAdjustUserBalance.builder().uid(TestConstants.UID_2).transactionId(1L).amount(20_000_00L).currency(TestConstants.CURRENCY_USD).build());
        cmds.add(ApiAdjustUserBalance.builder().uid(TestConstants.UID_2).transactionId(2L).amount(1_0000_0000L).currency(TestConstants.CURRENCY_XBT).build());
        cmds.add(ApiAdjustUserBalance.builder().uid(TestConstants.UID_2).transactionId(3L).amount(1_0000_0000L).currency(TestConstants.CURRENCY_ETH).build());
