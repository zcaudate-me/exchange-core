...
import kmi.exchange.beans.api.ApiCancelOrder;
...
public final class ITFeesExchange {
    ...
    private final long step = TestConstants.SYMBOLSPECFEE_XBT_LTC.quoteScaleK;
    private final long makerFee = TestConstants.SYMBOLSPECFEE_XBT_LTC.makerFee;
    private final long takerFee = TestConstants.SYMBOLSPECFEE_XBT_LTC.takerFee;
    ...
    public void shouldRequireTakerFees_GtcCancel() throws Exception {
        ...
        try (final ExchangeTestContainer container = new ExchangeTestContainer()) {
            ...
            final ApiPlaceOrder order203 = ApiPlaceOrder.builder().uid(TestConstants.UID_2).id(203).price(11_400).reservePrice(11_400).size(30).action(OrderAction.BID).orderType(GTC).symbol(TestConstants.SYMBOL_EXCHANGE_FEE).build();
            ...
            container.submitCommandSync(
                    ApiCancelOrder.builder().id(203).uid(TestConstants.UID_2).symbol(TestConstants.SYMBOL_EXCHANGE_FEE).build(),
                    CommandResultCode.SUCCESS);

            container.validateUserState(
                    TestConstants.UID_2,
                    userProfile -> assertThat(userProfile.accounts.get(TestConstants.CURRENCY_LTC), is(ltcAmount + takerFee * 30)),
                    orders -> assertTrue(orders.isEmpty()));

            TotalCurrencyBalanceReportResult totalBal1 = container.totalBalanceReport();
            assertThat(totalBal1.getSum().get(TestConstants.CURRENCY_LTC), is(ltcAmount + takerFee * 30));
            assertThat(totalBal1.getFees().get(TestConstants.CURRENCY_LTC), is(0L));
            ...
            final ApiPlaceOrder order204 = ApiPlaceOrder.builder().uid(TestConstants.UID_2).id(204).price(11_400).reservePrice(11_400).size(100).action(OrderAction.ASK).orderType(GTC).symbol(TestConstants.SYMBOL_EXCHANGE_FEE).build();
            container.submitCommandSync(order204, CommandResultCode.SUCCESS);

            // cancel ask
            container.submitCommandSync(
                    ApiCancelOrder.builder().id(204).uid(TestConstants.UID_2).symbol(TestConstants.SYMBOL_EXCHANGE_FEE).build(),
                    CommandResultCode.SUCCESS);

            container.validateUserState(
                    TestConstants.UID_2,
                    userProfile -> assertThat(userProfile.accounts.get(TestConstants.CURRENCY_XBT), is(btcAmount)),
                    orders -> assertTrue(orders.isEmpty()));

            // no fees collected
            TotalCurrencyBalanceReportResult totalBal2 = container.totalBalanceReport();
            assertThat(totalBal2.getSum().get(TestConstants.CURRENCY_LTC), is(ltcAmount + takerFee * 30));
            assertThat(totalBal2.getSum().get(TestConstants.CURRENCY_XBT), is(btcAmount));
            assertThat(totalBal2.getFees().get(TestConstants.CURRENCY_LTC), is(0L));
            assertThat(totalBal2.getFees().get(TestConstants.CURRENCY_XBT), is(0L));
        }
    }
    ...
    public void shouldProcessFees_BidGtcMaker_AskIocTakerPartial() throws Exception {
        ...
        try (final ExchangeTestContainer container = new ExchangeTestContainer()) {
            ...
            container.createUserWithMoney(TestConstants.UID_1, TestConstants.CURRENCY_LTC, ltcAmount); // 200B litoshi (2,000 LTC)
            ...
            container.validateUserState(
                    TestConstants.UID_1,
                    userProfile -> assertThat(userProfile.accounts.get(TestConstants.CURRENCY_LTC), is(expectedFundsLtc)),
            ...
            // no fees collected
            TotalCurrencyBalanceReportResult totalBal1 = container.totalBalanceReport();
            assertThat(totalBal1.getSum().get(TestConstants.CURRENCY_LTC), is(ltcAmount));
            assertThat(totalBal1.getSum().get(TestConstants.CURRENCY_XBT), is(btcAmount));
            assertThat(totalBal1.getFees().get(TestConstants.CURRENCY_LTC), is(0L));
            ...
            container.validateUserState(
                    TestConstants.UID_1,
                    userProfile -> {
                        assertThat(userProfile.accounts.get(TestConstants.CURRENCY_LTC), is(ltcAmount - (order101.price * step + makerFee) * 1731L));
                        assertThat(userProfile.accounts.get(TestConstants.CURRENCY_XBT), is(1731L * TestConstants.SYMBOLSPECFEE_XBT_LTC.baseScaleK));
                    },
                    orders -> assertTrue(orders.isEmpty()));
            ...
            container.validateUserState(
                    TestConstants.UID_2,
                    userProfile -> {
                        assertThat(userProfile.accounts.get(TestConstants.CURRENCY_LTC), is((order101.price * step - takerFee) * 1731L));
                        assertThat(userProfile.accounts.get(TestConstants.CURRENCY_XBT), is(btcAmount - 1731L * TestConstants.SYMBOLSPECFEE_XBT_LTC.baseScaleK));
                    },
                    orders -> assertTrue(orders.isEmpty()));

            // total balance remains the same
            final TotalCurrencyBalanceReportResult totalBal2 = container.totalBalanceReport();
            assertThat(totalBal2.getSum().get(TestConstants.CURRENCY_LTC), is(ltcAmount));
            assertThat(totalBal2.getSum().get(TestConstants.CURRENCY_XBT), is(btcAmount));
            assertThat(totalBal2.getFees().get(TestConstants.CURRENCY_LTC), is((makerFee + takerFee) * 1731L));
        }
    ...
    public void shouldProcessFees_BidGtcMakerPartial_AskIocTaker() throws Exception {
        ...
        try (final ExchangeTestContainer container = new ExchangeTestContainer()) {
            ...
            container.validateUserState(
                    TestConstants.UID_1,
                    userProfile -> {
                        assertThat(userProfile.accounts.get(TestConstants.CURRENCY_LTC),
                                is(ltcAmount - (order101.price * step + makerFee) * 1000L - (order101.reservePrice * step + takerFee) * 731L));
                        assertThat(userProfile.accounts.get(TestConstants.CURRENCY_XBT), is(1000L * TestConstants.SYMBOLSPECFEE_XBT_LTC.baseScaleK));
                    },
            ...
            container.validateUserState(
                    TestConstants.UID_2,
                    userProfile -> {
                        assertThat(userProfile.accounts.get(TestConstants.CURRENCY_LTC), is((order101.price * step - takerFee) * 1000L));
                        assertThat(userProfile.accounts.get(TestConstants.CURRENCY_XBT), is(btcAmount - 1000L * TestConstants.SYMBOLSPECFEE_XBT_LTC.baseScaleK));
                    },
                    orders -> assertTrue(orders.isEmpty()));
            ...
            assertThat(totalBal2.getSum().get(TestConstants.CURRENCY_LTC), is(ltcAmount));
            assertThat(totalBal2.getSum().get(TestConstants.CURRENCY_XBT), is(btcAmount));
            assertThat(totalBal2.getFees().get(TestConstants.CURRENCY_LTC), is((makerFee + takerFee) * 1000L));
        }
    ...
    public void shouldProcessFees_AskGtcMaker_BidIocTakerPartial() throws Exception {
        ...
        try (final ExchangeTestContainer container = new ExchangeTestContainer()) {
            ...
            container.validateUserState(
                    TestConstants.UID_1,
                    userProfile -> {
                        assertThat(userProfile.accounts.get(TestConstants.CURRENCY_XBT), is(0L));
                        assertThat(userProfile.accounts.get(TestConstants.CURRENCY_LTC), is((11_500L * step - makerFee) * 2000L));
                    },
                    orders -> assertTrue(orders.isEmpty()));
            ...
            container.validateUserState(
                    TestConstants.UID_2,
                    userProfile -> {
                        assertThat(userProfile.accounts.get(TestConstants.CURRENCY_XBT), is(TestConstants.SYMBOLSPECFEE_XBT_LTC.baseScaleK * 2000L));
                        assertThat(userProfile.accounts.get(TestConstants.CURRENCY_LTC), is(ltcAmount - (11_500L * step + takerFee) * 2000L));
                    },
                    orders -> assertTrue(orders.isEmpty()));
            ...
            assertThat(totalBal2.getSum().get(TestConstants.CURRENCY_LTC), is(ltcAmount));
            assertThat(totalBal2.getSum().get(TestConstants.CURRENCY_XBT), is(btcAmount));
            assertThat(totalBal2.getFees().get(TestConstants.CURRENCY_LTC), is((makerFee + takerFee) * 2000L));
        }
    ...
    public void shouldProcessFees_AskGtcMakerPartial_BidIocTaker() throws Exception {
        ...
        try (final ExchangeTestContainer container = new ExchangeTestContainer()) {
            ...
            container.validateUserState(
                    ...
                    userProfile -> {
                        assertThat(userProfile.accounts.get(TestConstants.CURRENCY_XBT), is(0L));
                        assertThat(userProfile.accounts.get(TestConstants.CURRENCY_LTC), is((11_500L * step - makerFee) * 1997L));
                    },
                    orders -> assertFalse(orders.isEmpty()));
            ...
            container.validateUserState(
                    TestConstants.UID_2,
                    userProfile -> {
                        assertThat(userProfile.accounts.get(TestConstants.CURRENCY_XBT), is(TestConstants.SYMBOLSPECFEE_XBT_LTC.baseScaleK * 1997L));
                        assertThat(userProfile.accounts.get(TestConstants.CURRENCY_LTC), is(ltcAmount - (11_500L * step + takerFee) * 1997L));
                    },
                    orders -> assertTrue(orders.isEmpty()));
            ...
            assertThat(totalBal2.getSum().get(TestConstants.CURRENCY_LTC), is(ltcAmount));
            assertThat(totalBal2.getSum().get(TestConstants.CURRENCY_XBT), is(btcAmount));
            assertThat(totalBal2.getFees().get(TestConstants.CURRENCY_LTC), is((makerFee + takerFee) * 1997L));
        }
    }
